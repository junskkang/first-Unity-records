...
{
    public int m_AttackRange;   //사정거리


    public Archer_Stat(string a_CrName = "", int a_AttackRange = 0)   //생성자 오버로딩 함수
    {
        m_CharType = CharicType.Archer;
        m_StrJob = "아처";
        m_Name = a_CrName;
        m_RscFile = "Images/ArcRenderer";  //해당 리소스를 사용하겠다는 의미
        m_MaxHp = 70;
        m_MaxMana = 80;
        m_Attack = 80;
    ...
    public override void UseSkill()
    {
        if (m_RefGameMgr != null)
            m_RefGameMgr.LogMsg($"아처 스킬 : 이름({m_ChrStat.m_Name}), " +
                                $"이속({((Archer_Stat)m_ChrStat).m_AttackRange})");
    }
}
