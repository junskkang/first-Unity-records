...
public enum PacketType
{
    ...
    GetRankingPointList,    //포인트 기준 랭킹 받아오기
...
public class NetworkMgr : G_Singleton<NetworkMgr>
{
    ...
    void Req_Network()  //RequestNetwork
    {
        ...
        else if (m_PacketBuff[0] == PacketType.GetRankingPointList)
            StartCoroutine(LobbyNetCom.GetRankPointListCo());
        ...
        else if (m_PacketBuff[0] == PacketType.MyPoint)
            StartCoroutine(InGameNetCom.UpdatePointCo());
    ...
    //public IEnumerator SendRequestWithTimeout(UnityWebRequest request,
    //            System.Action onSuccess, System.Action<string> onError)
    //{
    //    bool isTimeOut = false;
    //    float startTime = Time.unscaledTime;

    //    m_NetWaitTimer = m_Timeout;

    //    request.SendWebRequest();  // 요청 시작

    //    // 요청이 완료되거나 타임아웃이 발생할 때까지 대기
    //    while (!request.isDone && !isTimeOut)
    //    {
    //        if (Time.unscaledTime - startTime > m_Timeout)
    //        {
    //            isTimeOut = true;
    //        }
