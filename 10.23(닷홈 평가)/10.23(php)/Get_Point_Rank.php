<?php
	...
	$sqlList = mysqli_query($con, "SELECT * FROM RockScissorPaper ORDER BY mypoint DESC LIMIT 0, 10");
	// 0에서부터 10명까지만...
...
//	  	      	WHERE `user_id`='{$u_id}' ");
	...
	$check = mysqli_query($con, "SELECT user, myrankidx 
				FROM (SELECT user, 
				rank() over(ORDER BY mypoint DESC) as myrankidx 
			FROM RockScissorPaper) as CNT 
			WHERE user='{$u_id}' ");
