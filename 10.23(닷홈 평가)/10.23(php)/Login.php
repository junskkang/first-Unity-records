...
<?
	...
	$check = mysqli_query($con, "SELECT * FROM RockScissorPaper WHERE user='{$u_id}'");
	...
	$row = mysqli_fetch_assoc($check);	//user 이름에 해당하는 행의 내용을 가져온다.
	if($row){
		if($u_pw == $row["pass"])	//비밀번호도 맞다면
		{
