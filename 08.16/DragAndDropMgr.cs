...
public class DragAndDropMgr : MonoBehaviour
{
    ...
    string m_SvStrJson = "";        //서버에 전달하려고 하는 JSON 형식이 뭔지?

    bool isNetworkLock = false;
    float m_BuyDelayTime = 0.0f;
    string BuyRequestUtl = "";
    //--- 서버와의 통신을 위한 변수

    // Start is called before the first frame update
    void Start()
    {
        BuyRequestUtl = "http://http://junskk.dothome.co.kr/practice/Buy_Request.php";
    ...
    void MouseBtnUp()
    {
        ...
        if(isNetworkLock == true || 0.0f < m_BuyDelayTime)
        {  //아직 구매 할 수 없는 상태
            m_SaveIndex = -1;
            m_MsObj.gameObject.SetActive(false);
            ShowMessage("아직 이전 구매 처리 중입니다. 잠시 후 다시 시도해 주세요.");
            return;
        }
    ...
    IEnumerator BuyRequestCo()
    {
        ...
        m_SvStrJson = JsonUtility.ToJson(a_ItList);
        //Debug.Log(m_SvStrJson);
        //--- JSON 만들기...
        ...
        UnityWebRequest a_www = UnityWebRequest.Post(BuyRequestUtl, form);
        ...
        if(a_www.error == null)  //에러 나지 않았다면...
        {
            ...
            //m_SvMyPoint, m_BuySkType을 가지고 갱신하는 방법이 있다.)
